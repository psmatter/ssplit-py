...
Regex::
Regex(std::string pattern, uint32_t options, uint32_t jit_options)
...
std::string Regex::get_error_message() const {
  ...
  msg << "PCRE2 compilation failed at offset " << error_offset_ << ": " << buffer;
  return msg.str();
}

// return compiled regex
pcre2_code const*
Regex::get_pcre2_code() const {
  return re_;
}

int
Regex::consume(string_view* subj, // the string (view) agains we are matching
...
int
Regex::find(string_view subj, // the string (view) agains we are matching
     Match* M,               // where to store the results of the match
  ...
  int rc = pcre2_match(re_,                     /* the compiled pattern */
                       ...
                       options,                 /* options */
                       M->match_data,           /* block for storing result */
...
Match::Match(Regex const& re)
  : Match(re.get_pcre2_code())
{}

Match::Match(pcre2_code const* re)
...
string_view Match::operator[](int i) const {
