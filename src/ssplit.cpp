...
// Load a prefix sile
int SentenceSplitter::load(const char *fname){
...
uint32_t SentenceSplitter::sentences(){
	return  sentences_;
}
void SentenceSplitter::sentences(uint32_t s){
	sentences_=s;
}

SentenceSplitter::SentenceSplitter(){ sentences_=0; }
/*
SentenceSplitter::SentenceSplitter(char *prefix_file)
  : SentenceSplitter() {
  if (NULL!=prefix_file){
    load(prefix_file);
  }
...
SentenceStream(string_view text,
               ...
               bool verify_utf8)
  : SentenceStream(text.data(), text.size(), splitter, verify_utf8) {}

SentenceStream::
SentenceStream(char const* data, size_t datasize,
               SentenceSplitter const& splitter,
               bool verify_utf8)
  : cursor_(data), stop_(data + datasize), splitter_(splitter)
{
 ...
 string s = string(paragraph_.begin(), paragraph_.end());
 cerr << "SentenceStream:" << s << endl;
...
bool
SentenceStream::operator>>(string_view& snt){
...
bool
SentenceStream::operator>>(std::string& snt){
  string_view s;
  if (!((*this) >> s))
    return false;
